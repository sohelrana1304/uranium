...
const deleteAPI = async function (req, res) {
  try {
    ...
    if (!decodeToken) return res.send({ status: false, msg: "token is invalid" });

    let userId = req.params.userId;
    let user = await userModel.findById(userId);
    if (!user) return res.send({ msg: "no such user exists" })

    let updateUser = await userModel.findOneAndUpdate({ _id: userId }, { isDeleted: true }, { new: true, upsert: true })
    res.send({ status: updateUser, msg: updateUser })
  }
  catch (error) {
    console.log("This is an error:", error.message)
    res.status(500).send({ msg: "error", error: error.message })
